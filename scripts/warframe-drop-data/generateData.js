...
request(config.dropDataUrl, (err, res, body) => {
    ...
    for(let relic of data.relics) {
        ...
        relicList.push(relicStruct[relic.tier][relic.relicName])
    }
    ...
    console.log(`Writing... /data/relicsCompiled.json`)
    fs.writeFileSync(path.resolve(__dirname, `data`, `relicsCompiled.json`), JSON.stringify(relicList, null, jsonFormat))
    fs.writeFileSync(path.resolve(__dirname, `../..`, `data`, `relicsCompiled.json`), JSON.stringify(relicList, null, jsonFormat))
